...
from Crypto.Random import get_random_bytesimport streamlit as st
import re
import unicodedata
import requests
import json
...
class ChatModel:
    ...
    def smart_summarize(self, text, query):
        """
        خوارزمية التلخيص الذكي:
        تستخدم LLM لإعادة صياغة وتلخيص النص المقتبس من ويكيبيديا
        ليكون أكثر تركيزاً على السؤال وأسهل للقراءة.
        """
        # إذا كان النص قصيراً جداً، لا داعي للتلخيص
        if len(text) < 300:
            return text
        ...
        try:
            # هندسة الأمر لضمان عدم إضافة معلومات خارجية
            prompt = f"""
            You are a summarization engine. Your task is to summarize the provided text based on the user's query.
            
            Constraint:
            1. Use ONLY the information in the provided text. Do not add external knowledge.
            2. Keep the answer in the same language as the text.
            ...
            # في حال فشل التلخيص، نعيد النص الأصلي
            return text

    def search_wikipedia(self, query, lang):
        """
        بحث متقدم في ويكيبيديا مع خوارزمية التلخيص الذكي
        """
        try:
            target_title = None
            is_priority_topic = False
            
            # --- المرحلة 1: التحقق من القائمة المحددة (Priority Check) ---
            ...
            # --- المرحلة 2: البحث العام (General Search) ---
            if not target_title:
            ...
            # --- المرحلة 3: جلب المحتوى وتلخيصه (Fetch & Summarize) ---
            if target_title:
                ...
                if summary_response.status_code == 200:
                    ...
                    if extract:
                        # تطبيق خوارزمية التلخيص الذكي على النص المستخرج
                ...
                # إذا كان الموضوع من القائمة المحددة وفشل الجلب، نتوقف هنا ولا نذهب لـ Gemini
...
def chat_interface():
    ...
    if q := st.chat_input("اكتب سؤالك هنا..."):
        ...
        with st.status("جاري المعالجة والبحث...", expanded=True) as status:
            ...
            if not ans:
                status.write("🌐 البحث في المصادر المفتوحة (Wikipedia) + تلخيص ذكي...")
                wiki_ans, topic = model.search_wikipedia(model.normalize_text(q), lang)
                if wiki_ans:
                    ans, src = wiki_ans, topic
                    status.update(label="تم جلب وتلخيص المعلومات من ويكيبيديا", state="complete", expanded=False)
            
            # 3. Gemini AI (Only if not found in Priority Wiki topics)
            if not ans:
                # تمت إزالة رسالة الحالة الخاصة بـ Gemini كما طلب سابقاً
